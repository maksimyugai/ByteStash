...
import { SnippetRepository, type Snippet } from '../repositories/snippetRepository.js';
...
function summarizeSnippet(snippet: Snippet) {
  return {
    id: snippet.id,
    title: snippet.title,
...
function fullSnippet(snippet: Snippet) {
  return {
    id: snippet.id,
    title: snippet.title,
    description: snippet.description,
...
export function createMcpServer(db: D1Database, userId: number) {
  const snippets = new SnippetRepository(db);
  ...
  server.registerTool(
    ...
    async (args) => {
      try {
        ...
        const { snippets: results, total } = await snippets.findAllPaginated({
          userId,
          filters: {
        ...
        return ok({
          snippets: results.map(summarizeSnippet),
  ...
  server.registerTool(
    ...
    async (args) => {
      try {
        const snippet = await snippets.findById(args.id, userId);
        if (!snippet) {
  ...
  server.registerTool(
    ...
    async (args) => {
      try {
        const existing = await snippets.findById(args.id, userId);
        if (!existing) {
          return fail(`Snippet ${args.id} not found`);
        }
        ...
        const updated = await snippets.update(
          args.id,
          {
            ...
            fragments,
            isPublic: (args.is_public ?? !!existing.is_public) ? 1 : 0,
          },
          userId
        );
    ...
    async (args) => {
      try {
        const result = args.permanent
          ? await snippets.delete(args.id, userId)
          : await snippets.moveToRecycle(args.id, userId);
  ...
  server.registerTool(
    'list_metadata',
    {
      ...
      description:
        "List all category/tag names and languages used across the user's snippets, plus a total count. Useful for building filters.",
      inputSchema: {},
    },
    async () => {
      try {
        const metadata = await snippets.getMetadata(userId);
        return ok(metadata);
      } catch (error) {
